    ...
    server.config.scrims.forEach((scrim) => {
      ...
      if (msg.channel.id === scrim.channel_Confirmations) {
        ...
        if (msg.member.roles.cache.find(role => role.name === server.config.role_BotAdmin)) {
          ...
          let slot = msg.content.split(" ")[1] == "all" ? "all"  : parseInt(msg.content.split(" ")[1]);
          ...
          if (slot === "all") {
            ...
            client.channels.fetch(scrim.channel_Confirmations).then(channel => {
              ...
              channel.messages.fetch(scrim.msg_Slots).then(message => {
                ...
                client.channels.fetch(server.config.channel_Logs).then(channel => {
                  ...
                  channel.send("Saving slots for records:\n\n" + message.content).catch(err => console.error("Error happened."));
            ...
            client.channels.fetch(server.config.channel_Logs).then(channel => {
              ...
              setTimeout(() => {
                ...
                channel.send("Removed **__" + scrim.role_IDPW + "__** from **" + membersIDPW + "** members.").catch(err => console.error("Error happened.")).catch(err => console.error("Error happened."));
            ...
            client.channels.fetch(server.config.channel_Logs).then(channel => {
              ...
              channel.send("Removing **__" + scrim.role_Waitlist + "__** from all members, please wait **" + roleWAIT.members.size + "** secs.").catch(err => console.error("Error happened."));

              setTimeout(() => {

                channel.send("Removed **__" + scrim.role_Waitlist + "__** from **" + membersWAIT + "** members.").catch(err => console.error("Error happened."));
            ...
            if (slot >= (scrim.max_number_of_teams + scrim.number_first_slot)) {
              ...
              msg.channel.send("Cleared slot " + slot +
              ...
              console.log("Done removing slot");
          ...
          client.commands.get('%slots').execute(msg, server, client, true);
